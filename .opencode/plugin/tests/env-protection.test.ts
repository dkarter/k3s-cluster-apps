...
describe('env protection', () => {
  test('registers a V2 before-execution hook and blocks protected inputs', async () => {
    ...
    const run = (tool: string, input: Record<string, string>) =>
      before!({ tool, input });

    expect(() => run('read', { path: '/project/.env.local' })).toThrow(
      'Do not read .env files',
    );
    expect(() => run('read', { filePath: '/project/.env' })).toThrow(
      'Do not read .env files',
    );
    expect(() => run('shell', { command: 'cat .env' })).toThrow(
      'Do not read .env files',
    );
    expect(() => run('grep', { include: '**/.env*' })).toThrow(
      'Do not read .env files',
    );
    expect(() => run('shell', { command: 'kubectl get secret' })).toThrow(
      'Do not read kubernetes secrets',
    );
    expect(() => run('shell', { command: 'gh auth token' })).toThrow(
      'Do not read github tokens',
    );
    expect(() =>
      run('shell', { command: 'gh auth status --show-token' }),
    ).toThrow('Do not read github tokens');
    expect(() => run('shell', { command: 'gcloud secrets list' })).toThrow(
      'Do not read gcloud secrets',
    );
